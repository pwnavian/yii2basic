...
class ActionColumn extends Column
{
    ...
    protected function initDefaultButtons()
    {
        ...
        $this->contentOptions = ['style'=>'display:flex; gap: 10px;justify-content:center'];
        // pr($this->contentOptions);exit;
    ...
    protected function initDefaultButton($name, $iconName, $additionalOptions = [])
    {
        if (!isset($this->buttons[$name]) && strpos($this->template, '{' . $name . '}') !== false) {
            ...
            // pr($model);exit;
            if(sizeof($tops) > 2) {
                $url = Url::to([$tops[2].'/'.$tops[3].'/'.$name.'/'.$key]);
            }
            // pr($name);exit;
            $options = array_merge([
                'title' => $title,
            ...
            $options = array_merge(['class'=>"btn-putera"], $additionalOptions);
            // $icon = isset($this->icons[$iconName])
            //     ? $this->icons[$iconName]
            //     : Html::tag('span', '', ['class' => "glyphicon glyphicon-$iconName"]);
            
            return Html::a(strtoupper($name), $url, $options);
            };
        }
    }
    ...
    public static function createTooltip($model){
        $str = '';
        foreach($model->tooltipInfo() as $key => $val){
            $str.= $val.' : ' . $model->{$key} . ' ';
        }
        return $str;
    }
