    ...
    protected function renderFieldSet()
    {
        ...
        for ($row = 1; $row <= $rows; $row++) {
            ...
            for ($col = 1; $col <= $cols; $col++) {
                ...
                // pr($width);exit;
                Html::addCssClass($colOptions, 'col-' . $this->columnSize . '-' . ($cols == 2 ? $colWidth-1 : $colWidth));
                ...
                if($cols ==2 && $col == 1) $content .='<div class="col-sm-2">&nbsp;</div>';
                $index++;
            }
            $content .= $this->endTag('div', $skip) . "\n";
        }
